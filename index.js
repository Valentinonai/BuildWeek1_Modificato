...
const controllaLive = () => {
  const rispUtente = document.querySelectorAll(".divAnswer");
  rispUtente.forEach((x) => (x.disabled = true));

  rispUtente.forEach((x) => {
    if (x.innerText === questions[questionNumber - 1].correct_answer) {
...
const controlla = () => {
  let c = false;
  let answer = document.querySelectorAll(".divAnswer");
  console.log(risposteUtente, risposteCorrette);
  answer.forEach((x) => {
    console.log(x.classList.contains("seleziona"));
    if (x.classList.contains("seleziona") === true) {
      c = true;
      risposteUtente.push(x.innerText); //!Variabili per riepilogo risposte
...
const seleziona = (event) => {
  const risp = document.querySelectorAll(".divAnswer");
  risp.forEach((x) => x.classList.remove("seleziona"));
  event.target.classList.add("seleziona");
};
const generaDomanda = () => {
  ...
  if (questionNumber < parseInt(numDomande)) {
    ...
    const answerGroup1 = document.createElement("div");
    const answerGroup2 = document.createElement("div");
    for (let i = 0; i < x; i++) {
      const posDomanda = Math.floor(Math.random() * arrayDomande.length);
      const divAnswer = document.createElement("button");
      divAnswer.classList.add("divAnswer");

      divAnswer.innerText = arrayDomande[posDomanda];

      arrayDomande.splice(posDomanda, 1);
      if (i === 0 || i === 1) {
        answerGroup1.appendChild(divAnswer);
      } else answerGroup2.appendChild(divAnswer);
    }
    answerGroup.classList.add("answerGroup");

    answerGroup.appendChild(answerGroup1);
    answerGroup.appendChild(answerGroup2);
    quiz.appendChild(answerGroup);

    const listaRisposte = document.querySelectorAll(".divAnswer");
    listaRisposte.forEach((x) => x.addEventListener("click", seleziona));
    console.log(listaRisposte);
